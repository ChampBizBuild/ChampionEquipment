-- Champion Equipment — FULL SETUP (safe to re-run)
...
insert into document_templates (type, title, body)
select * from (values
(
  'terms_of_trade',
  'Terms of Trade',
...
where not exists (select 1 from document_templates t where t.type = v.type);
...
create table if not exists bookings (
  ...
  notes text,
  hire_details jsonb not null default '{}'::jsonb,
...
-- Extra columns if bookings table already existed without them
alter table bookings add column if not exists hire_details jsonb not null default '{}'::jsonb;
...
alter table bookings add column if not exists needs_service boolean not null default false;
alter table bookings add column if not exists accept_token text;
...
create index if not exists bookings_dates_idx on bookings(pickup_date, dropoff_date);
...
do $$ begin
  create policy "operators all business_settings" on business_settings for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators all document_templates" on document_templates for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators all clients" on clients for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators all equipment" on equipment for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators all bookings" on bookings for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators all documents" on documents for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators all invoices" on invoices for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;
...
create index if not exists condition_inspections_booking_idx on condition_inspections (booking_id);
create index if not exists condition_inspections_token_idx on condition_inspections (client_ack_token);

alter table condition_inspections enable row level security;

do $$ begin
  create policy "operators all condition_inspections" on condition_inspections for all to authenticated using (true) with check (true);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators read documents bucket" on storage.objects for select to authenticated using (bucket_id = 'documents');
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators write documents bucket" on storage.objects for insert to authenticated with check (bucket_id = 'documents');
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "operators update documents bucket" on storage.objects for update to authenticated using (bucket_id = 'documents');
exception when duplicate_object then null; end $$;

-- Seed excavator only if none exist
insert into equipment (name, make_model, asset_id, day_rate, week_rate, status, notes)
select '1.8T Excavator', 'Kubota / equivalent', 'MRX371', 320.00, 1400.00, 'available', 'Starter plant record'
where not exists (select 1 from equipment limit 1);

update equipment set asset_id = 'MRX371'
where asset_id is null and (name ilike '%1.8%' or name ilike '%excavator%');
update equipment set asset_id = 'MRX364'
where asset_id is null and (name ilike '%259%' or make_model ilike '%259%');
